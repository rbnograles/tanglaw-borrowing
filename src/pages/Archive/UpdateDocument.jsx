...
const UpdateDocument = ({ 
    ...
    const callUpdateDocument = async () =>{
        try{
            const results = await updateDocument(selectedDocument, document, file);
            const logs = await createSystemLogs("Updated a document.", loggedInProfile.email, loggedInProfile.userType);
    ...
    const submit = () => {
        if(document.fileName !== selectedDocument.fileName && file === null){
            setOpenSnackbar(true);
            setMessage(`Can't change File Name if file is empty`);
            setSnackbarType('error');
            return;
        }
        if(document.department !== selectedDocument.department && file === null){
            setOpenSnackbar(true);
            setMessage(`Can't change Department if file is empty`);
            setSnackbarType('error');
            return;
        }
        if(document === selectedDocument && file === null){
            setOpenSnackbar(true);
