...
import CreateStudent from './utilities/Create';
import EditDepartment from "./utilities/EditDepartment";
import DeleteStudentAccount from './utilities/Delete';
...
const Student = () => {
    ...
    const { addNewFacultyAccount, getAllStudent, deleteUserProfile, editFacultyAccount, getUserProfile  } = UserProfile();
    ...
    const handleCreateAccount = async (formValues) => {
        try {
            ...
            // prepare data object for creating the student account
            const value = {
            ...
            if(data.success) {
                ...
                setIsLoading(false)
                handleClose();
                await createSystemLogs("Created a student account.", loggedInProfile.email, loggedInProfile.userType);
                _getAllStudent();
            }
        } catch(error) {
            setIsLoading(false);
    ...
    const handleAccountEdits = async (e) => {
        ...
        try {
            ...
            if(res.success) {
                await createSystemLogs("Edited a student account.", loggedInProfile.email, loggedInProfile.userType);
                setSnackbarType('success');
    ...
    const handleAccountDepartmentEdit = async () => {
        ...
        try {
            ...
            if(res.success) {
                setIsLoading(false)
                await createSystemLogs("Edited student department.", loggedInProfile.email, loggedInProfile.userType);
                setSnackbarType('success');
                setMessage('Cell data has been updated succesfully')
                setOpenSnackbar(true);
                handleCloseEdit();
    ...
    const deleteRowItem = async () => {
        ...
        try {
            ...
            if(res2.success) {
                await createSystemLogs("Deleted a student account.", loggedInProfile.email, loggedInProfile.userType);
                _getAllStudent();
                setSnackbarType('success');
        ...
        <Fragment>
            <div className='flex justify-between mb-2 mt-3'>
                <p className='tableWarnings'>
                    <b>Notice:</b> Double click a cell in order to edit. Cells with <LockIcon style={{ fontSize: "17px"}}/> <b>is not editable.</b>
                </p>
                <div className='flex'>
                    <Button 
                        sx={{ marginRight: "10px" }} 
