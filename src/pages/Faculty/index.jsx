...
import CreateFaculty from './utilities/Create';
import EditDepartment from "./utilities/EditDepartment";
import DeleteFacultyAccount from './utilities/Delete';
...
const Faculty = () => {
    ...
    const { addNewFacultyAccount, getAllFaculty, deleteUserProfile, editFacultyAccount, getUserProfile  } = UserProfile();
    const { createSystemLogs } = SystemLogs();

     // get all faculty and set to state
    const _getAllFaculty = async () => {
        try {
    ...
    const handleCreateAccount = async (formValues) => {
        try {
            ...
            if(data.success) {
                ...
                setIsLoading(false)
                handleClose();
                await createSystemLogs("Created a faculty account.", loggedInProfile.email, loggedInProfile.userType);
                _getAllFaculty();
            }
        } catch(error) {
            setIsLoading(false);
    ...
    const handleAccountEdits = async (e) => {
        ...
        try {
            ...
            if(res.success) {
                await createSystemLogs("Edited a faculty account.", loggedInProfile.email, loggedInProfile.userType);
                setSnackbarType('success');
    ...
    const deleteRowItem = async () => {
        ...
        try {
            ...
            if(res2.success) {
                await createSystemLogs("Deleted a faculty account.", loggedInProfile.email, loggedInProfile.userType);
                _getAllFaculty();
                setSnackbarType('success');
