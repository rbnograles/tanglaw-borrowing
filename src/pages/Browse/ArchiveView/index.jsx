...
const ArchiveView = () => {
  ...
  const { getAllCommentsPerArticle, createCommentOnArticle, deleteComment } =
    Comments();
  ...
  return (
    <ResponsiveDrawer>
      <PageTitle title={`Article from the Course of ${document.department}`} />
        ...
        <Box style={{ width: "60%", marginRight: "30px" }}>
          ...
          <Stack direction="row" spacing={1} className="mr-2 mb-3">
            {document.authors.map((author, i) => {
              return <Chip icon={<FaceIcon />} key={i} label={author} />;
            })}
          </Stack>
          ...
          <p>
            <span className="inline-text-decor">Date Uploaded</span>:{" "}
            <Moment format="DD MMMM YYYY">{document.publicationDate}</Moment> |{" "}
            <span className="inline-text-decor">Production Year:</span>{" "}
            {document.schoolYear} |{" "}
            <span className="inline-text-decor">Citations:</span>{" "}
          ...
          <Stack direction="row" spacing={1} className="mr-2">
            {document.keywords.map((keyword, i) => {
              return <Chip key={i} label={keyword} />;
            })}
          </Stack>
          ...
          <div className="mt-2 mb-5">
            ...
            <Stack direction="row" spacing={1} className="mr-2">
              {document.formats.map((format, i) => {
                return <Chip key={i} label={format} />;
              })}
            </Stack>
            <br />
            ...
            <Stack direction="row" spacing={1} className="mr-2">
              {document.languages.map((language, i) => {
                return <Chip key={i} label={language} />;
              })}
            </Stack>
            <br />
            ...
            {document.physicalDescription.map((description, i) => {
              return (
                <p className="small-content" key={i}>
                  {i + 1}. {description}
                </p>
              );
            })}
        ...
        <Box className="comment-panel">
          ...
          <Box className="comment-content">
            ...
            {newComment.length === 0 && (
              <Box
                style={{ height: "100%" }}
                className="flex p-4 justify-center text-center items-center"
              >
                <p>No Comments</p>
              </Box>
            )}
